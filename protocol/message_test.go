...
func TestControlMessageCoder(t *testing.T) {
	for _, test := range []struct {
		...
		},
		{
			...
			want: []byte{0x00, 0x07, 0x03, 0x04, 0x49, 0x76, 0x61, 0x6e, 0x04, 0x09, 0x49, 0x76, 0x61, 0x6e, 0x6f, 0x76, 0x69, 0x63, 0x68,
		...
		},
		{
			...
			want: []byte{0x00, 0x07, 0x06, 0x07, 0x02, 0x07, 0xd6},
		},
		{
			name: "Test_UserList",
			input: ControlMessage{
			...
			want: []byte{0x00, 0x08, 0x01, 0x09, 0x03, 0x08, 0x01, 0x04, 0x50, 0x65, 0x74, 0x72, 0x0f, 0x01, 0x82, 0x04, 0x00, 0x04,
		...
		},
		{
			...
			want: []byte{0x00, 0x09, 0x82, 0x04, 0x00, 0x82, 0x01, 0x00},
		},
		{
			...
			want: []byte{0x00, 0x0a, 0x0a, 0x82, 0x01, 0x00},
		},
	} {
		c := &test.input
		res := Coder(c)
...
func TestControlMessageDecoder(t *testing.T) {
	for _, test := range []struct {
		...
		{
			name: `Test_UserInfo`,
			input: []byte{0x00, 0x07, 0x03, 0x04, 0x49, 0x76, 0x61, 0x6e, 0x04, 0x09, 0x49, 0x76, 0x61, 0x6e, 0x6f, 0x76, 0x69, 0x63, 0x68,
				0x05, 0x06, 0x49, 0x76, 0x61, 0x6e, 0x6f, 0x76, 0x06, 0x07, 0x02, 0x07, 0xd6},
			want: ControlMessage{
				Type: UserInfoMessageType,
				UserInfo: &UserInfo{
		...
		{
			name:  "Test_UserInfo_2",
			input: []byte{0x00, 0x07, 0x06, 0x07, 0x02, 0x07, 0xd6},
			want: ControlMessage{
				Type: UserInfoMessageType,
				UserInfo: &UserInfo{
					Name:       "",
		...
		{
			name: "Test_UsersList",
			input: []byte{0x00, 0x08, 0x01, 0x09, 0x03, 0x08, 0x01, 0x04, 0x50, 0x65, 0x74, 0x72, 0x0f, 0x01, 0x82, 0x04, 0x00, 0x04,
				0x49, 0x76, 0x61, 0x6e, 0xff, 0x04, 0x0a, 0x04, 0x4f, 0x6c, 0x67, 0x61, 0xf0},
			want: ControlMessage{
				Type: UsersListMessageType,
				UsersList: UsersList{
		...
		{
			name:  "Test_MessagesRequest",
			input: []byte{0x00, 0x09, 0x82, 0x04, 0x00, 0x82, 0x01, 0x00},
			want: ControlMessage{
				Type:   MessagesRequestMessageType,
				RoomId: 1024,
				MessagesRequest: MessagesRequest{
		...
		{
			name:  "Test_NumberOfMessages",
			input: []byte{0x00, 0x0a, 0x0a, 0x82, 0x01, 0x00},
			want: ControlMessage{
				Type:             NumberOfMessagesMessageType,
				RoomId:           10,
				NumberOfMessages: 256,
